...
export class GeolocateComponent implements OnInit, OnDestroy {
  ...
  loadEntityCounts() {
    ...
    const summaryPromise = this.ztService.get('summary', paging, []);
  ...
  loadMapData() {
    ...
    const identitiesPromise = this.ztService.get('identities', paging, []).then((result) => {
      this.identitiesInit = true;
      this.addMarkers(result?.data, 'identity');
    });
    const routersPromise = this.ztService.get('routers', paging, []);
    const edgeRoutersPromise = this.ztService.get('edge-routers', paging, [])
