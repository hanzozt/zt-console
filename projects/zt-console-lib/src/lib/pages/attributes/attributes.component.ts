...
import { ZitiDataService, ZITI_DATA_SERVICE } from '../../services/zt-data.service';
...
export class AttributesComponent implements OnInit, OnDestroy {
  ...
  loadData() {
      ...
      promises.push(this.ztDataService.get('identities', {}, []).then(data => {
          this.identities = data?.data || [];
      }));
      promises.push(this.ztDataService.get('services', {}, []).then(data => {
          this.services = data?.data || [];
      }));
      promises.push(this.ztDataService.get('edge-routers', {}, []).then(data => {
          this.edgeRouters = data?.data || [];
      }));
      promises.push(this.ztDataService.get('posture-checks', {}, []).then(data => {
          this.postureChecks = data?.data || [];
      }));
      promises.push(this.ztDataService.get('identity-role-attributes', {}, []).then(data => {
          this.identityAttributes = data?.data || [];
      }));
      promises.push(this.ztDataService.get('edge-router-role-attributes', {}, []).then(data => {
          this.edgeRouterAttributes = data?.data || [];
      }));
      promises.push(this.ztDataService.get('service-role-attributes', {}, []).then(data => {
