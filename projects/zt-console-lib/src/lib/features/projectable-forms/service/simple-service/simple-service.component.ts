...
export class SimpleServiceComponent extends ProjectableForm {
  ...
  override ngOnInit() {
    ...
    this.ztSessionId = this.settingsService?.settings?.session?.id;
    this.initFormData();
  ...
  getAssociatedConfigs(): Promise<any> {
    return this.ztService.getSubdata('services', this.formData.id, 'configs').then((result: any) => {
  ...
  getAssocaitedPolicies(): Promise<any> {
    return this.ztService.getSubdata('services', this.formData.id, 'service-policies').then((result: any) => {
  ...
  getIdentityNamedAttributes(filter?) {
    ...
    this.ztService.get('identities', paging, filters).then((result) => {
  ...
  async checkNameConflicts() {
    ...
    const paging = cloneDeep(this.ztService.DEFAULT_PAGING);
    paging.noSearch = false;
    const serviceProm = this.ztService.get('services', paging, [filter]).then((result) => {
    ...
    const interceptProm = this.ztService.get('configs', paging, [filter]).then((result) => {
      if (result.data?.length > 0) {
    ...
    const hostProm = this.ztService.get('configs', paging, [filter]).then((result) => {
      if (result.data?.length > 0) {
    ...
    const dialProm = this.ztService.get('service-policies', paging, [filter]).then((result) => {
      if (result.data?.length > 0) {
    ...
    const bindProm = this.ztService.get('service-policies', paging, [filter]).then((result) => {
      if (result.data?.length > 0) {
  ...
  async saveInterceptConfig(summaryData) {
    ...
    if (this.sdkOnlyDial && !isNewConfig) {
      return this.ztService.delete('configs', this.interceptConfigApiData.id);
  ...
  async saveHostConfig(summaryData) {
    ...
    // Conditional Deletion (Identical Logic)
    if (this.sdkOnlyDial && !isNewConfig) {
      return this.ztService.delete('configs', this.hostConfigApiData.id);
    } else if (this.sdkOnlyDial && isNewConfig) {
      return;
    }
  ...
  async saveConfig(
    ...
    if (configResult.error) {
      ...
      this[cliCommandProp] = `zt edge create config '${configApiData.name}' '${configApiData.configTypeId}' '${JSON.stringify(configApiData.data)}'`;
  ...
  async saveService(summaryData) {
    ...
    if (serviceResult.error) {
      ...
      this.serviceCliCommand = `zt edge create service '${this.serviceApiData.name}' ${cliConfigIds}`
  ...
  async savePolicy(summaryData, type = 'Dial') {
    ...
    const cliCommand = `zt edge create service-policy '${policyApiData.name}' ${type} --semantic AnyOf --service-roles '@${this.serviceId}' --identity-roles '${cliRoles}'`;
    if (type === 'Dial') {
  ...
  saveEntity(type, data) {
    return this.ztService.post(type, data, true);
  }

  updateEntity(type, data) {
    return this.ztService.patch(type, data, data.id, true);
  }
