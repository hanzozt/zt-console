...
export class TreeNodeProcessor {
    ...
    // fetch associated routers
      ...
      const associatedRoutersPromise =  await ztService.get(url, paging, [])
           .then( (result) => {
      ...
      const firstPromise =  await ztService
        ...
        identityServicePolicies.forEach( (sp) => {
             const nodeArray =  this.getServicesForAServicePolicy(sp, ztService, 500);
  ...
  getLinkForResource(ob, resource) {
    ...
    findServicesHostedOnEndpoint(rawEp, services, servicePolicies, ztService) {
       const servicesArr = [];
  ...
  async processServicesForNodeClick(serviceNode, networkGraph, services, uniqId, ztService) {
     const myPromise = await new Promise( async (resolve, reject) => {
        const wait_promises = [];
        this.uniqId = uniqId;
        ...
        const configPromise = await ztService
       ...
       const ser_promse =  await ztService
       ...
       const promise2 = await ztService
  ...
  async processServicePoliciesForNodeClick(policyNode, networkGraph,  servicePolicies, uniqId, ztService) {
     const myPromise = await new Promise( async (resolve, reject) => {
        const wait_promises = [];
        this.uniqId = uniqId;
        ...
        const sp_promise = await ztService.get(bindIdentitiesUrl, identities_paging, [])
       .then((ids) => {
         const childs =new Children()
       ...
       const _promise = await ztService.get(bindServicesUrl, services_paging, [])
       .then((result) => {
         const childs =new Children()
   ...
   async getServicesForAServicePolicy(policyObject, ztService, pagesize=500) {
       const childs = [];
       const services_paging = this.getPagingObject(pagesize);
   ...
   async processEdgeroutersForNodeClick(
    ...
    )
    {
     const mainPromise = await new Promise( async (resolve, reject) => {
       ...
       let erpoliciesForER = [];
       const erpoliciespromise = await ztService.get(erpoliciesUrl, services_paging, [])
       .then( async (result) => {
          ...
          const promises = [];
           for (let page = 2; page <= pages; page++) {
              ...
              const tmp_promise = await ztService.get(
                               erpoliciesUrl,
                               services_paging,
                               []
                ) .then((pageResult) => {
    ...
    async processEdgeRouterPoliciesForNodeClick(
      ...
      const erChailds = new Children();
        ...
        erChailds.type= 'Edge Routers';
        erChailds.id = this.createId();
      const associatedRoutersPromise =  await ztService.get(er_url, paging, [])
           .then( (result) => {
          ...
          let ids = [];
          const idpromise = ztService.get(url, services_paging, [])
          .then((result) => {
            ids = result.data;
            const pages = Math.ceil(result.meta.pagination.totalCount / pagesize);
            const promises = [];
            for (let page = 2; page <= pages; page++) {
              services_paging.page = page;
              const tmp_promise = ztService.get(
                               url,
                               services_paging,
                               []
                ) .then((pageResult) => {
  ...
  async processServiceEdgeRouterPoliciesForNodeClick(
       ...
       const myPromise = await new Promise((resolve, reject) => {
          ...
          let routers = [];
          const erpromise = ztService.get(url, services_paging, [])
          .then((result) => {
            routers = result.data;
            const pages = Math.ceil(result.meta.pagination.totalCount / pagesize);
            const promises = [];
          ...
          let serArray = [];
          const serpromise = ztService.get(urlB, services_pagingB, [])
          .then((result) => {
            ...
            for (let page = 2; page <= pages; page++) {
              ...
              const tmp_promiseB = ztService.get(
                               urlB,
                               services_pagingB,
                               []
                ) .then((pageResult) => {
