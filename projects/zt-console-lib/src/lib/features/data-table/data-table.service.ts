...
export class DataTableService {
  ...
  refreshCells(gridObj, rowData) {
    ...
    if (dataChanged) {
      gridObj.api.ztAllToggled = !_.isEmpty(rowData) && _.every(rowData, {selected: true});
  ...
  saveColumnState() {
    ...
    localStorage.setItem(`zt_${this.tableId}_table_state`, tableStateCookie);
  }
  ...
  private saveColumnWidths(column) {
    ...
    let colWidthsCookie = localStorage.getItem(`zt_${this.tableId}_column_widths`);
    let colWidths: any = {};
    ...
    localStorage.setItem(`zt_${this.tableId}_column_widths`, colWidthsCookie);

    let tableStateCookie = localStorage.getItem(`zt_${this.tableId}_table_state`);
    let tableState;
  ...
  resetCookieConfig() {
    localStorage.removeItem(`zt_${this.tableId}_table_state`);
    localStorage.removeItem(`zt_${this.tableId}_column_widths`);
  }
